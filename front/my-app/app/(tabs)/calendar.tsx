...
interface Turno {
  ...
  reservado: boolean; // Añadido para saber si el turno está reservado
}
                ...
                .map((turno) => (
                  <View key={turno.id} style={styles.turnoCard}>
                    <View style={styles.row}>
                      <Text style={styles.hora}>{`${turno.hora_inicio} - ${turno.hora_fin}`}</Text>
                      <BotonReserva
                        turnoId={turno.id}
                        cuposDisponibles={turno.cupos_disponibles}
                        yaReservado={turno.reservado}  
                        onReservaExitosa={(nuevosCupos) => {
                          setTurnos((prev) =>
                            prev.map((t) =>
                              t.id === turno.id
                                ? { ...t, cupos_disponibles: nuevosCupos, reservado: true }
                                : t
                            )
                          );
                        }}
